...
const vcard = 'BEGIN:VCARD\n' // OM JANGAN DI UBAH OM
            ...
            + 'FN:RamlanGans\n' // HIDEKI MODD
            + 'ORG:Owner BOT;\n' // HIDEKI MODD
            + 'TEL;type=CELL;type=VOICE;waid=5511946462131:+55 11 946462131\n' // UBAH INI
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `[ BEM VINDO AO GRUPO ${mdata.subject}* ] \n___________________________\n@${num.split('@')[0]} Olá filha da puta! \nManda o número da mãe ae \nAqui é um grupo de fodase, então fodase? \nFodase? \nSabe chamar o bot? se não? fodase \nVai mamar o adm pra ganhar admin😲😲😃? \nNegro?😈😍 \nSALVA O NÚMERO DOS ADMS AE E AVISA NO PV! \n *___________________________*\nNÃO DESRESPEITE NENHUMA REGRA!`
				...
				teks = `flw, negro saiu do grupo, F rapaziada... @${num.split('@')[0]}😔✌* \n_Qualquer coisa manda nude pro adm pra voltar pro gp😍😈🙌`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					admin: '❌ 𝗔𝗽𝗲𝗻𝗮𝘀 𝗮𝗱𝗺𝘀 𝗽𝗼𝗱𝗲𝗺 𝘂𝘀𝗮𝗿 𝗲𝘀𝘀𝗲 𝗰𝗼𝗺𝗮𝗻𝗱𝗼, 𝗺𝗮𝗰𝗮𝗰𝗼 𝗮𝗿𝗿𝗼𝗺𝗯𝗮𝗱𝗼 ! ❌',
					Badmin: '❌𝗔𝗽𝗲𝗻𝗮𝘀 𝗮𝗱𝗺𝘀 𝗽𝗼𝗱𝗲𝗺 𝘂𝘀𝗮𝗿 𝗲𝘀𝘀𝗲 𝗰𝗼𝗺𝗮𝗻𝗱𝗼, 𝗺𝗮𝗰𝗮𝗰𝗼 𝗮𝗿𝗿𝗼𝗺𝗯𝗮𝗱𝗼, 𝗜𝗠𝗣𝗢𝗦𝗦𝗜𝗩𝗘𝗟 𝗗𝗘 𝗖𝗢𝗡𝗖𝗟𝗨𝗜𝗥 𝗢 𝗖𝗢𝗠𝗔𝗡𝗗𝗢!! ❌'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5511946462131@s.whatsapp.net"] // Ganti pake nomor lu
			const ramlanNumber = ["5511946462131@s.whatsapp.net"] // JANGAN DI UBAH NGENTOD
			...
			switch(command) {
				...
				case 'info':
					...
					teks = `*Hideki bot* : ${me.name}\n*Hideki Modd* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Total Block Contact* : ${blocked.length}\n*The bot is active on* : ${kyun(uptime)}`
					buffer = await getBuffer(me.imgUrl)
					client.sendMessage(from, buffer, image, {caption: teks, contextInfo:{mentionedJid: [me.jid]}})
					break
				case 'blocklist':
					teks = 'CONTATOS BLOQUEADOS DE QUANDO FUI DA KAMIBLOCK RS :\n'
               ...
               client.sendMessage(from, '𝗛𝗜𝗜𝗗𝗘𝗞𝗜 𝗟𝗜𝗡𝗗𝗢. <- 𝗠𝗘𝗨 𝗖𝗥𝗜𝗔𝗗𝗢𝗥!!:*',MessageType.text, { quoted: mek} )
                break
				case 'ocr':
				...
				case 'sticker':
					...
					break
				case 'tts':
					if (args.length < 1) return client.sendMessage(from, 'Cadê o código de idioma puta?', text, {quoted: mek})
					const gtts = require('./R4ML4N/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, 'Cadê o texto negro', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					dtt.length > 600
					? reply('Texto muito grande, máximo 300 ')
				...
				case 'setprefix':
					...
					prefix = args[0]
					reply(`Agora o prefix é esse ---> : ${prefix}`)
					break
				case 'hilih':
				...
				case 'bc':
					...
					if (mentioned.length > 1) {
						...
						mentions(`O @${mentioned[0].split('@')[0]} agora é negro supremo`, mentioned, true)
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('Quem vc quer add porra?')
					if (args[0].startsWith('08')) return reply('Codigo do país, burro')
					try {
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Vai dar ban em quem? na sua namorada? marca ai porra!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedidos recebidos, emitidos, toma esse ban negro😈😨 :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`KKKKKKJJJ levou ban 😲 : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'admin':
